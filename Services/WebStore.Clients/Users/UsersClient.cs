...
namespace WebStore.Clients.Users
{
    public class UsersClient : BaseClient, IUserClient
    {
        ...
        {
            await PostAsync($"{_ServiceAddress}/AddClaims", new AddClaimDTO { User = user, Claim = claims }, cancel);
        }
        ...
        {
            await PostAsync($"{_ServiceAddress}/RemoveClaims", new RemoveClaimDTO { User = user, Claim = claims },
                cancel);
        }
